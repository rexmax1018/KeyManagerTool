...
namespace KeyManagerTool.CryptoLib.Services
{
    public class DataEncryptionService : IDataEncryptionService
    {
        ...
        public string Encrypt(string plainText, string unifiedName)
        {
            ...
            SymmetricKeyModel aesKey = _cryptoKeyService.GenerateKeyOnly<SymmetricKeyModel>(CryptoAlgorithmType.AES);
            ...
            byte[] encryptedAesKeyAndIvBytes = _cryptoService.Encrypt(combinedAesKeyAndIvBytes, CryptoAlgorithmType.RSA, rsaPublicKeyModel);

            string encryptedDataPart = Convert.ToBase64String(encryptedDataBytes);
            string encryptedAesKeyIvPart = Convert.ToBase64String(encryptedAesKeyAndIvBytes);

            string combinedEncryptedDataAndKey = $"{encryptedDataPart}::{encryptedAesKeyIvPart}";

            string result = $"{combinedEncryptedDataAndKey}.{unifiedName}";
        ...
        public string Decrypt(string encryptedDataWithUnifiedName)
        {
            ...
            try
            {
                ...
                combinedEncryptedDataAndKey = encryptedDataWithUnifiedName.Substring(0, lastDotIndex);
                unifiedName = encryptedDataWithUnifiedName.Substring(lastDotIndex + 1);
            }
            catch (Exception ex)
            {
            ...
            int separatorIndex = combinedEncryptedDataAndKey.IndexOf("::");
            if (separatorIndex == -1)
            {
            ...
            string encryptedDataPart = combinedEncryptedDataAndKey.Substring(0, separatorIndex);
            string encryptedAesKeyIvPart = combinedEncryptedDataAndKey.Substring(separatorIndex + 2);
        ...
        public string GetUnifiedNameFromEncryptedData(string encryptedDataWithUnifiedName)
        {
            ...
            return encryptedDataWithUnifiedName.Substring(lastDotIndex + 1);
        }
